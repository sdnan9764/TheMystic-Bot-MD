 ...
 //   let apii = await this.getFile(pp)
  ...
  /* } else if (m.messageStubType == 28) {
   conn.sendMessage(m.chat, { text: `${usuario} ELIMINO A @${m.messageStubParameters[0].split`@`[0]} 🫵`, mentions: [m.sender]}, { quoted: fkontak })  */
    ...
    await this.sendMessage(m.chat, { text: `@${m.messageStubParameters[0].split`@`[0]} *تم ترقيتة ليصبح أدمن هاذي المجموعة*\n\n*لقد قام هاذا بترقيتة الى ادمن*: ${usuario}`, mentions: [`${m.sender}`,`${m.messageStubParameters[0]}`] }, { quoted: fkontak })
  } else if (m.messageStubType == 30) {
    await this.sendMessage(m.chat, { text: `@${m.messageStubParameters[0].split`@`[0]} *تم ازالتة من لائحة الادمن او المشرفين في هاذي المجموعة*\n\n*لقد قام هاذا بإزالة اشرافك*: ${usuario}`, mentions: [`${m.sender}`,`${m.messageStubParameters[0]}`] }, { quoted: fkontak })
